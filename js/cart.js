...
function cargar_producto_carrito(){
    ...
    if( productos_carrito && productos_carrito.length > 0 ){
        ...
        productos_carrito.forEach(productos => {
            ...
            div.innerHTML=  `
            <img class="carrito-producto-imagen" src="${productos.image}" alt="${productos.title}">
            ...
            <div class="carrito-producto-precio">
                <small>Precio</small>
                <p>${formatCurrency(productos.price)} </p>
            </div>
            <div class="carrito-producto-subtotal">
                <small>Subtotal</small>
                <p>${formatCurrency(productos.price * productos.cantidad)}</p>
            </div>
            <button class="carrito-producto-eliminar" id="${productos.id}"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
...
function eliminar_carrito(e){
    ...
    const index =productos_carrito.findIndex(productos=> productos.id == idBtn);
...
function vaciar_carrito(){
    Swal.fire({
        title: '¿Estás seguro?',
        icon: 'question',
        html: `Se van a borrar ${productos_carrito.reduce((acc, productos) => acc + productos.cantidad, 0)} productos.`,
 ...
 function actualizar_total (){
  const total_calculado =   productos_carrito.reduce((ac,productos) => ac + (productos.price * productos.cantidad), 0);
